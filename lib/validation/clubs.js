...
export const ClubSchema = (isEdit = false) => z.object({
  ...
  cwo_name: z.string().min(2, "Cwo Name is required"),
  cwo_phone: z.string()
    .trim()
    .min(10, { message: "Cwo PHone must be at least 10 digits." })
    .max(11, { message: "Cwo PHone must be at most 11 digits." })
    .regex(/^\d+$/, { message: " Digits only (0–9)" }),
  secretary_website: z.string().url("Enter a valid URL").optional().or(z.literal("")),
  phone: z.string()
    .trim()
  ...
  cwo_email: z.string().trim().superRefine((val, ctx) => {
    if (!val) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Email is required",
