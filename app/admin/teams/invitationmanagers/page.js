...
import { deleteManager } from "@/actions/managersInvitationActions";
...
export default function FanTable() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch("/api/teams/invitationmanagers");
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.$ && managers.length > 0) {
      const $ = window.$;
      ...
      <main className="main-body col-md-9 col-lg-9 col-xl-10">
        <div className="body-top d-flex flex-wrap justify-content-between align-items-center gap-20 mb-10">
          ...
          <div className="top-right d-flex justify-content-between align-items-center gap-10">
            <Link prefetch={false} className="btn btn-common" href="/admin/teams/invitationmanagers/new">New Manager Invitation</Link>
            <a href="#">
        ...
        <div className="body-title-bar d-flex flex-wrap justify-content-between align-items-center gap-20 mb-10">
          <div className="body-title-bar-left d-flex flex-wrap align-items-center gap-20-70">
            <h1 className="page-title">Managers Invitation</h1>
          </div>
        </div>
                ...
                {managers.length > 0 ? (
                  managers.map((l, index) => (
                    <tr key={l._id}>
                      ...
                      <td className="text-nowrap"><a href="tel:+44 07453 234258">{l.manager_phone}</a></td>
                      <td className="text-nowrap"><a href="mailto:csb9900@gmail.com">{l.manager_email}</a></td>
                      <td className="text-nowrap"><a href="#">{l.manager_nick_name}</a></td>
                      <td className="text-nowrap">{l.manager_address}</td>
                       <td className="text-nowrap">{l.manager_invitation_code}</td>
