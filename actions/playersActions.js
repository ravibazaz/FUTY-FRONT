...
export async function createPlayers(prevState, formData) {
  ...
  const filePath = path.join(process.cwd(), "uploads/players", uniqueName);
  ...
  cookieStore.set("toastMessage", "Player Added");
  redirect("/admin/players");
}

export async function updatePlayers(id, prevState, formData) {
  const raw = Object.fromEntries(formData.entries());
  const result = PlayersSchema(true).safeParse(raw);
  const cookieStore = await cookies();
  if (!result.success) {
    return { success: false, errors: result.error.flatten().fieldErrors };
  }
  ...
  if (imageFile && imageFile.size > 0) {
    const uploadsFolder = path.join(process.cwd(), "uploads/players");
